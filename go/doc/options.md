...
name inherits the fields it leaves unset (for example `{"hash": {EatLine: &t}}`
keeps the `#` start), and a `nil` def removes just that marker. An
explicit `Line: Bool(false)` is honored, so `{"hash": {Line: Bool(false),
End: "@@"}}` turns the default `#` line comment into a `# ... @@` block
comment (see [differences](differences.md#comment-definitions-commentdef)).

### `CommentDef`

| Field | Type | Description |
|---|---|---|
| `Line` | `*bool` | `true` for line comments, `false` for block. `nil` means not supplied: a default name keeps its setting and a new name gets a block comment. Write it with `Bool(true)` or `Bool(false)` |
