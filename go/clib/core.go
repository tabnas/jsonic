...
//
// libtabnasjsonic is the jsonic format parser as a C shared library, one of
...
import (
	...
	plug "github.com/tabnas/jsonic/go"
)

const (
	templateVersion = "v1"
	libName         = "libtabnasjsonic"
	formatName      = "jsonic"
	valueOut        = true
)
...
// newParser builds one engine with the jsonic grammar installed
...
func newParser() (parseFn, error) {
	j := plug.Make(); if _, err := j.Parse("true"); err != nil { return nil, err }; return j.Parse, nil
}
...
// parseWith answers whether src is in the jsonic language — and,
