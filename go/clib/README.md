# libtabnasjsonic — the jsonic parser as a C ABI

<!-- tabnas-clib-template: v1 — stamped by admin tasks/adopt-clib.sh;
     edit the template and re-stamp, not this file. -->

The jsonic format parser as a C shared library, so languages with no
tabnas port can parse and validate jsonic input. This is one of the
...
| Function | Returns |
|---|---|
| `tabnas_version()` | `{"ok":true,"lib":"libtabnasjsonic","format":"jsonic","template":"v1"}` |
...
// link against libtabnasjsonic; every call returns a JSON []u8 to free with
// c.tabnas_free.
```
