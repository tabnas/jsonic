...
publish-go: test-go
	...
	sed -i.bak 's/^const VERSION = ".*"/const VERSION = "$(V)"/' go/jsonic.go
	rm -f go/jsonic.go.bak
	git add go/jsonic.go
	git commit -m "go: v$(V)"
	git tag go/v$(V)
...
# release: the crate depends on the engine and the JSON core by path, and
# crates.io does not accept a path dependency, so tabnas-jsonic is not
