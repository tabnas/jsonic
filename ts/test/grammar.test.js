...
describe('grammar-options', () => {
  ...
  it('skip-sentinel-exported', () => {
    ...
    // It is the parsing engine's (tabnas) SKIP sentinel — jsonic
    // re-exports it so deep-merge / grammar-option semantics line up.
    assert.equal(typeof Jsonic.SKIP, 'symbol')
    assert.equal(Jsonic.SKIP, Symbol.for('tabnas.SKIP'))
  })
